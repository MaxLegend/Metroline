...
public class TunnelTypePopupMenu extends JWindow {

    private final Tunnel tunnel;
    private JPanel contentPanel;
    private static final java.util.Map<Object, TunnelTypePopupMenu> openWindows = new java.util.HashMap<>();
    private AWTEventListener outsideClickListener; // Для отслеживания кликов вне меню

    public TunnelTypePopupMenu(Tunnel tunnel, int screenX, int screenY) {
        super();

        this.tunnel = tunnel;

        // Закрыть существующее окно для этого туннеля
        if (openWindows.containsKey(tunnel)) {
            openWindows.get(tunnel).dispose();
        }
    ...
    private void initUI() {
        ...
        // Добавляем кнопки для каждого типа туннеля
    ...
    private JButton createTypeButton(TunnelType type) {
    ...
    private void positionWindow(int screenX, int screenY) {
        ...
        // Коррекция позиции для удержания в пределах экрана
    ...
    private void addAutoCloseListener() {
        ...
        // Закрытие при клике ВНЕ границ меню
